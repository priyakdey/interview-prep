...
class MyQueue:
    ...
    def pop(self) -> int:
        if len(self.stack) == 0:
            raise Exception("Illegal Operation: Empty Stack")

        while len(self.stack) != 0:
            self.swap_helper.append(self.stack[-1])
            del self.stack[-1]

        val = self.swap_helper[-1]
        del self.swap_helper[-1]

        while len(self.swap_helper) != 0:
            self.stack.append(self.swap_helper[-1])
            del self.swap_helper[-1]
