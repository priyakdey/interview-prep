...
class Solution:
    def sortTheStudents(self, score: List[List[int]], k: int) -> List[List[int]]:
        ...
        for row, _ in enumerate(score):
            score_to_row_map[score[row][k]] = row
            heap.heappush(scores, score[row][k])

        sorted_scores = []
        while len(scores) != 0:
            sc = heap.heappop(scores)
            sorted_scores.append(score[score_to_row_map[sc]])

        return list(reversed(sorted_scores))
